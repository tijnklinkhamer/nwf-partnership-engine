/**
 * PHASE 2B-2D2C-F0I — THE SHARED CORE for loading, hash-verifying and
 * DERIVING from an ATTEMPT-3 configuration freeze (PROPOSED, preparation
 * only - no owner freeze approval and no execution authorisation exists).
 *
 * Modelled deliberately on `attempt2FreezeCore.ts`, which stays byte-for-byte
 * unmodified: F0E and attempt 2 remain immutable historical evidence, and a
 * SEPARATE module for attempt 3 is what keeps that true structurally rather
 * than by convention. Every schema rule, cross-check and derivation here is
 * the SAME shape as attempt 2's, with exactly two generalisations:
 *
 *   1. The scheduled candidate is `PROMPT_V4_CANONICAL` (attempt 2's schema
 *      hard-codes `PROMPT_V3_CANONICAL` as a literal, so it cannot load an
 *      attempt-3 freeze without this parallel module).
 *   2. Each batch now carries a SECOND read-only comparator map,
 *      `attempt2ComparatorFinalInputSha256`, alongside the unchanged
 *      `attempt1ComparatorFinalInputSha256` - because attempt 3 compares
 *      against BOTH preserved attempts, never reruns either.
 *
...
/** The attempt-1 and attempt-2 authorisations, both CONSUMED; neither can authorise attempt 3. */
...
export const ATTEMPT_3_VARIANT_LABEL = 'PROMPT_V4_CANDIDATE';

/** The attempt-1 variants that must NEVER appear in an attempt-3 plan. */
export const ATTEMPT_1_VARIANT_NAMES = ['PROMPT_V1_CANONICAL', 'PROMPT_V2_CANONICAL'] as const;
/** The attempt-2 variant that must NEVER be re-run for attempt 3 - only read as a comparator. */
export const ATTEMPT_2_VARIANT_NAME = 'PROMPT_V3_CANONICAL';

/** The identity of the one attempt-3 variant, as the revision pins it. */
export interface Attempt3VariantIdentity {
  readonly name: typeof ATTEMPT_3_VARIANT_NAME;
  readonly label: typeof ATTEMPT_3_VARIANT_LABEL;
...
/** Everything that distinguishes one attempt-3 freeze revision from another. Only one exists so far: F0I. */
export interface Attempt3FreezeRevision {
  readonly label: 'F0I';
  ...
  readonly variant: Attempt3VariantIdentity;
...
const Attempt3VariantSchema = z.strictObject({
  name: z.literal(ATTEMPT_3_VARIANT_NAME),
  label: z.literal(ATTEMPT_3_VARIANT_LABEL),
...
export const Attempt3BatchSchema = z.strictObject({
  ordinal: z.int().min(1),
  ...
  finalInputSha256: z.strictObject({ PROMPT_V4_CANONICAL: Sha256 }),
...
export function createAttempt3FreezeSchema(rev: Attempt3FreezeRevision) {
  return z.looseObject({
    freezeId: z.literal(rev.freezeId),
    version: z.literal(rev.version),
    status: z.literal(ATTEMPT_3_STATUS),
    ...
    /** Both preserved attempts, byte-unchanged, never superseded (attempt 3 is a DIFFERENT attempt, not a correction). */
    predecessor: z.looseObject({
      ...
      attempt2: z.looseObject({
        ...
        // Attempt 2 is the first attempt with a repair round, so its evidence
        // inventory is measured as TWO separate hashes (primary vs repair-1),
        // exactly as F0H's own closure table reports it - never combined into
        // one fabricated value that was never actually computed.
    ...
    batching: z.looseObject({
      ...
      /** V1, V2 AND V3 - all three prior variants - are read-only comparators for attempt 3, never rerun. */
      priorVariantsNotScheduled: z.tuple([
        z.literal('PROMPT_V1_CANONICAL'),
        z.literal('PROMPT_V2_CANONICAL'),
        z.literal('PROMPT_V3_CANONICAL'),
      ]),
      plan: z.array(Attempt3BatchSchema).length(EXPECTED_LOGICAL_BATCHES_PER_VARIANT),
    }),
    inputConstruction: z.looseObject({
      ...
      promptVersionByVariant: z.strictObject({ PROMPT_V4_CANONICAL: z.string().min(1) }),
    }),
    classifier: z.looseObject({
      ...
      variants: z.array(Attempt3VariantSchema).length(1),
    ...
    scoring: z.looseObject({
      ...
      comparatorPolicy: z.looseObject({
        attempt1: z.looseObject({
          ...
          neverInsideAttempt3Namespace: z.literal(true),
        }),
        attempt2: z.looseObject({
...
export type Attempt3Freeze = z.infer<ReturnType<typeof createAttempt3FreezeSchema>>;
export type Attempt3Batch = z.infer<typeof Attempt3BatchSchema>;

export class Attempt3FreezeError extends Error {
  override readonly name = 'Attempt3FreezeError';
  constructor(
...
export interface LoadedAttempt3Freeze {
  readonly freeze: Attempt3Freeze;
  readonly rawSha256: string;
  readonly rawBytes: number;
  readonly revision: Attempt3FreezeRevision;
}
 ...
 * revision descriptor.
 */
export function loadAttempt3FreezeFromBytes(
  rev: Attempt3FreezeRevision,
  bytes: Buffer,
): LoadedAttempt3Freeze {
  const rawSha256 = sha256Hex(bytes);
  if (rawSha256 !== rev.rawSha256) {
    throw new Attempt3FreezeError(
  ...
  const result = createAttempt3FreezeSchema(rev).safeParse(parsed);
  if (!result.success) {
  ...
  assertAttempt3FreezeAgreesWithProduction(rev, result.data);
...
export function assertAttempt3FreezeAgreesWithProduction(
  rev: Attempt3FreezeRevision,
  ...
  if (
    freeze.inputConstruction.promptVersionByVariant.PROMPT_V4_CANONICAL !== pinned.promptVersion
  ) {
    problems.push('promptVersionByVariant');
  }
  if (freeze.git.v4Runtime.commit !== pinned.gitCommit) problems.push('git.v4Runtime.commit');
  if (freeze.git.v4Runtime.basedOn !== pinned.runtimeBaseCommit)
    problems.push('git.v4Runtime.basedOn');
  if (freeze.git.freezeBranchBasedOn !== rev.freezeBranchBasedOn)
    problems.push('git.freezeBranchBasedOn');

  // Repair policy: the production constants, exactly (unchanged from attempt 2 - R1 is untouched).
  ...
  // Liveness: unchanged from attempt 2 and equal to production.
  ...
  for (const [index, batch] of freeze.batching.plan.entries()) {
    ...
    const expectedV4 = computeFinalInputSha256({
    ...
    if (batch.finalInputSha256.PROMPT_V4_CANONICAL !== expectedV4)
      problems.push(`plan ${batch.ordinal}: V4 final identity`);
    for (const attempt1 of Object.values(batch.attempt1ComparatorFinalInputSha256)) {
      if (attempt1 === expectedV4)
        problems.push(`plan ${batch.ordinal}: V4 identity equals an attempt-1 identity`);
    }
    if (batch.attempt2ComparatorFinalInputSha256.PROMPT_V3_CANONICAL === expectedV4) {
      problems.push(`plan ${batch.ordinal}: V4 identity equals the attempt-2 identity`);
    }
...
/** THE mechanical ceiling: identical formula to attempt 2's - one original request plus at most one repair per document. */
...
export interface Attempt3PlannedEvaluation {
  readonly sequence: number;
  readonly attemptNo: 3;
  readonly variantName: 'PROMPT_V4_CANONICAL';
  readonly variantLabel: 'PROMPT_V4_CANDIDATE';
  ...
  readonly batchContext: Attempt3Batch['context'];
...
export interface Attempt3ExecutionPlan {
  ...
  readonly repairPolicy: Attempt3Freeze['repairPolicy'];
  readonly liveness: {
  ...
  readonly callCeilingTotals: Attempt3Freeze['callCeiling']['totals'];
  readonly evaluations: readonly Attempt3PlannedEvaluation[];
}

/**
 * The attempt-3 plan: exactly the 12 frozen batches, in ordinal order, for
 * the ONE variant PROMPT_V4_CANONICAL. No attempt-1 or attempt-2 variant,
 * root, identity or artifact enters it; their comparator identities stay in
 * the freeze as provenance only.
 */
export function buildAttempt3ExecutionPlan(
  freeze: Attempt3Freeze,
  freezeRawSha256: string,
): Attempt3ExecutionPlan {
  const variant = freeze.classifier.variants[0]!;
  const evaluations: Attempt3PlannedEvaluation[] = freeze.batching.plan.map((batch, index) => ({
    sequence: index + 1,
    attemptNo: ATTEMPT_3_NO,
    variantName: ATTEMPT_3_VARIANT_NAME,
    variantLabel: ATTEMPT_3_VARIANT_LABEL,
    variantOrder: 1,
    ...
    finalInputSha256: batch.finalInputSha256.PROMPT_V4_CANONICAL,
...
/** Stable identity of an attempt-3 plan: SHA-256 of its canonical serialization. */
export function attempt3PlanSha256(plan: Attempt3ExecutionPlan): string {
  return sha256Hex(canonicalStringify(plan));
}

/** True iff the plan is the ONE V4 variant over ordinals 1..12 in order, and no V1/V2/V3 variant appears. */
export function attempt3PlanOrderIsFrozen(plan: Attempt3ExecutionPlan): boolean {
  const priorVariantNames: readonly string[] = [...ATTEMPT_1_VARIANT_NAMES, ATTEMPT_2_VARIANT_NAME];
  return (
    ...
    plan.evaluations.every(
      (evaluation, index) =>
        ...
        evaluation.variantName === ATTEMPT_3_VARIANT_NAME &&
        !priorVariantNames.includes(evaluation.variantName),
    )
  );
}

/** `<outputRoot>/experiments/attempt-3` - never created by this module; naming only. */
export function attempt3ExperimentDirectoryOf(outputRoot: string): string {
  return `${outputRoot}/experiments/attempt-${ATTEMPT_3_NO}`;
}

/** The write-once attempt-3 namespace shape; never created by this module. */
export function attempt3AttemptDirectoryOf(
  outputRoot: string,
  logicalBatchOrdinal: number,
): string {
  return `${outputRoot}/evaluations/${ATTEMPT_3_VARIANT_NAME}/batch-${String(logicalBatchOrdinal).padStart(2, '0')}/attempt-${ATTEMPT_3_NO}`;
}
