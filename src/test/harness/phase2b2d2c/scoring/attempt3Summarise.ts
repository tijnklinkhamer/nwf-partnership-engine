/**
 * PHASE 2B-2D2C-F0J — THE DETERMINISTIC ATTEMPT-3 SUMMARY.
 *
 * Everything here is a function of the verified sources: the attempt-3
 * PROMPT_V4_CANONICAL rows (POST-REPAIR validity, per ADR 0011 and the F0I
 ...
 * and the F0I freeze's own gates. First-pass validity is ALWAYS reported
 * beside the post-repair figures and is never hidden - for BOTH the
 * candidate and the V3 comparator.
 *
...
/** Versions the attempt-3 SCORING RULES. Any change to how a row or a summary is derived changes this. */
export const ATTEMPT3_SCORER_VERSION = 'phase2b-2d2c-f0j-attempt3-scorer-v1';
/** Versions the attempt-3 derived-output SHAPE. */
export const ATTEMPT3_OUTPUT_SCHEMA_VERSION = 'phase2b-2d2c-f0j-attempt3-summary-v1';
...
export interface Attempt3RepairSummary {
...
export interface ComparatorMovement {
  readonly comparator: 'PROMPT_V1_CANONICAL' | 'PROMPT_V2_CANONICAL' | 'PROMPT_V3_CANONICAL';
  ...
  readonly recoveredByV4: readonly string[];
  readonly lostByV4: readonly string[];
...
export interface Attempt3Summary {
  ...
  readonly repair: Attempt3RepairSummary;
...
function comparatorMovementOf(
  comparator: 'PROMPT_V1_CANONICAL' | 'PROMPT_V2_CANONICAL' | 'PROMPT_V3_CANONICAL',
...
export function buildAttempt3Summary(input: {
  ...
  const { attempt3, attempt1Comparator, attempt2Comparator, v4Rows } = input;
  const v4Evaluations = attempt3.evaluations;
  const first = v4Rows[0];
  const rounds = v4Evaluations.flatMap((e) => (e.repairRound === null ? [] : [e.repairRound]));
  const sumRounds = (select: (round: (typeof rounds)[number]) => number): number =>
    rounds.reduce((total, round) => total + select(round), 0);
  const firstPass = validityCountsOf(v4Rows, 'FIRST_PASS');
  const postRepair = validityCountsOf(v4Rows, 'POST_REPAIR');
  ...
  const frozenGates: Readonly<Record<string, number>> = Object.fromEntries(
    Object.entries(attempt3.freeze.scoring.gates).filter(
      (entry): entry is [string, number] => typeof entry[1] === 'number',
    ),
  );
  const semanticMetrics = goldIsAvailable
    ? semanticMetricsOf('PROMPT_V4_CANONICAL' satisfies ScoredVariantName, v4Rows, frozenGates)
    : null;
  ...
  const perBatchCost: PerBatchCost[] = attempt3.plan.evaluations.map((planned) => {
    ...
    return {
      ...
      v4: {
        wallTimeMs: v4?.wallTimeMs ?? null,
        inputTokens: v4?.inputTokens ?? null,
        outputTokens: v4?.outputTokens ?? null,
        repairWallTimeMs: v4?.repairRound?.monotonicWallTimeMs ?? 0,
        repairExecuted: v4?.repairRound?.executed ?? 0,
      },
  ...
  return {
    scorerVersion: ATTEMPT3_SCORER_VERSION,
    outputSchemaVersion: ATTEMPT3_OUTPUT_SCHEMA_VERSION,
    attemptNo: 3,
    sources: {
      attempt3: {
        freezeVersion: attempt3.freeze.version,
        freezeRawSha256: attempt3.freezeRawSha256,
        planSha256: attempt3.planSha256,
        artifactsVerified: attempt3.artifactsVerified,
        artifactInventorySha256: attempt3.artifactInventorySha256,
        repairArtifactInventory: attempt3.repairArtifactInventory,
        consumptionMarkerSha256: attempt3.consumptionMarkerSha256,
        authorisationSha256: attempt3.authorisationSha256,
        consumedAtUtc: attempt3.consumedAtUtc,
        experimentStatus: attempt3.experimentStatus,
        experimentCompletedAtUtc: attempt3.experimentCompletedAtUtc,
      },
      attempt1Comparator: {
      ...
      corpus: {
        rawSha256: attempt3.corpusRawSha256,
        manifestRawSha256: attempt3.corpusManifestRawSha256,
        contentSha256: attempt3.corpusContentSha256,
        items: attempt3.corpusRows.length,
        split: 'DEVELOPMENT',
      },
    },
    ...
    candidate: {
      ...
      predictionDistribution: distributionOf(v4Rows),
      inputTokens: sumOf(v4Evaluations, (e) => e.inputTokens),
      outputTokens: sumOf(v4Evaluations, (e) => e.outputTokens),
      wallTimeMs: sumOf(v4Evaluations, (e) => e.wallTimeMs),
    },
    repair: {
      ...
      artifactInventory: attempt3.repairArtifactInventory,
      artifactsVerified: attempt3.repairArtifactsVerified,
      policy: {
        enabled: attempt3.freeze.repairPolicy.enabled,
        minimumRemainingBudgetMs: attempt3.freeze.repairPolicy.minimumRemainingBudgetMs,
      },
    },
    ...
    unmeasuredGates,
    devGateOutcome,
    pageVersusUnitFailures: pageVersusUnitFailuresOf(v4Rows),
    comparisons: [
      comparatorMovementOf('PROMPT_V1_CANONICAL', input.pairedV1ToV4),
      comparatorMovementOf('PROMPT_V2_CANONICAL', input.pairedV2ToV4),
      comparatorMovementOf('PROMPT_V3_CANONICAL', input.pairedV3ToV4),
    ],
    perBatchCost,
    holdoutEligibility: {
      holdoutInferencePermittedByThisSummary: false,
      rule: attempt3.freeze.scoring.acceptanceRule,
      devCandidatePassesEveryFrozenGate: goldIsAvailable
        ? devGateOutcome === 'ALL_FROZEN_GATES_MET_ON_DEV'
        : null,
    },
    stochasticCaveat: attempt3.freeze.scoring.comparatorPolicy.stochasticCaveat,
    scope:
