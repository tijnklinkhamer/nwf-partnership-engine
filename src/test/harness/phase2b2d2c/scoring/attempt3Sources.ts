/**
 * PHASE 2B-2D2C-F0J — READ-ONLY LOADING AND INTEGRITY VERIFICATION OF A
 * FUTURE ATTEMPT-3 ROOT, PINNED TO THE APPROVED+RATIFIED F0I FREEZE.
 *
 * This is the attempt-3 counterpart of `sources.ts`/`attempt2Sources.ts`: it
 * opens the F0I freeze (by exact hash), the DEVELOPMENT-only canonical
 * corpus, and an attempt-3 output root, and verifies — through the SAME
 * per-evaluation loader attempt 1 and attempt 2 are held to — that the root
 * holds exactly the twelve frozen PROMPT_V4_CANONICAL evaluations of
 * attempt 3 and nothing else:
 *
 *   - exactly one variant directory, `PROMPT_V4_CANONICAL`, with exactly
 *     twelve batch directories, each holding exactly `attempt-3`;
 *   - NO `PROMPT_V1_CANONICAL`, `PROMPT_V2_CANONICAL` or
 *     `PROMPT_V3_CANONICAL` directory: an attempt-1 or attempt-2 artifact
 *     placed into the attempt-3 namespace is refused;
 *   - exactly one experiment directory, `attempt-3`, completed for all
 *     twelve evaluations, with the completion naming the one variant;
 *   - exactly one consumption marker, for attempt 3, whose hash is the one
 *     the experiment manifest recorded and is NEITHER spent attempt-1 NOR
 *     spent attempt-2 authorisation;
 *   - every planned identity equal to the approved F0I plan (rebuilt here,
 *     its SHA-256 equal to the approved value), every artifact
 *     hash-verified, every repair round re-verified against the frozen
 *     document.
 *
 ...
 * file's import graph). When no attempt-3 root exists — the state at F0J —
...
export const ATTEMPT3_EXPECTED_PRIMARY_ARTIFACT_COUNT =
  EXPECTED_LOGICAL_BATCHES_PER_VARIANT * 10 + 2 + 1;

export interface LoadedAttempt3Sources {
  readonly attemptNo: 3;
  readonly freeze: F0IFreeze;
  ...
  readonly plan: F0IExecutionPlan;
...
/**
 * Loads and verifies every attempt-3 source. Throws on the first integrity
 * or identity failure; a partially-verified run never becomes a metric.
 */
export function loadAttempt3ScoringSources(
  repoRoot: string,
  outputRoot: string,
): LoadedAttempt3Sources {
  const loadedFreeze = loadF0IFreezeFromBytes(readFileSync(join(repoRoot, F0I_FREEZE_PATH)));
  if (loadedFreeze.rawSha256 !== PROPOSED_F0I_FREEZE_RAW_SHA256) {
    fail(
      `the F0I freeze hashes to ${loadedFreeze.rawSha256}; the pinned value is ${PROPOSED_F0I_FREEZE_RAW_SHA256}.`,
    );
  }
  ...
  if (mismatches.length > 0) {
    fail(`reconstructed batches differ from the F0I freeze: ${mismatches.join(', ')}.`);
  }
  const plan = buildF0IExecutionPlan(freeze, loadedFreeze.rawSha256);
  if (!f0iPlanOrderIsFrozen(plan)) fail('the rebuilt attempt-3 plan is not in the frozen order.');
  const rebuiltPlanSha256 = f0iPlanSha256(plan);
  if (rebuiltPlanSha256 !== PROPOSED_F0I_PLAN_SHA256) {
    fail(
      `rebuilt attempt-3 plan SHA-256 ${rebuiltPlanSha256} differs from the pinned ${PROPOSED_F0I_PLAN_SHA256}.`,
    );
  }

  // Namespace: exactly the attempt-3 shape, and nothing of attempt 1 or attempt 2.
  const topLevel = readdirSync(outputRoot).sort();
  for (const entry of topLevel) {
    if (!['authorisations', 'evaluations', 'experiments'].includes(entry)) {
      fail(`the attempt-3 root holds an unexpected entry ${entry}.`);
    }
  }
  const experimentDirs = readdirSync(join(outputRoot, 'experiments')).sort();
  if (experimentDirs.length !== 1 || experimentDirs[0] !== `attempt-${ATTEMPT_3_NO}`) {
    fail(
      `experiments/ holds ${experimentDirs.join(', ') || '(none)'}; exactly attempt-${ATTEMPT_3_NO} is expected.`,
    );
  }
  const variantDirs = readdirSync(join(outputRoot, 'evaluations')).sort();
  if (variantDirs.length !== 1 || variantDirs[0] !== ATTEMPT_3_VARIANT_NAME) {
    fail(
      `evaluations/ holds ${variantDirs.join(', ') || '(none)'}; exactly ${ATTEMPT_3_VARIANT_NAME} is expected (attempt-1/attempt-2 variants are never inside the attempt-3 namespace).`,
    );
  }
  const batchDirs = readdirSync(join(outputRoot, 'evaluations', ATTEMPT_3_VARIANT_NAME)).sort();
  ...
  if (JSON.stringify(batchDirs) !== JSON.stringify(expectedBatchDirs)) {
    fail(
      `evaluations/${ATTEMPT_3_VARIANT_NAME} holds ${batchDirs.join(', ')}; the twelve frozen batches are expected.`,
    );
  }

  // Experiment-level artifacts and the consumption marker.
  const experimentDirectory = join(outputRoot, 'experiments', `attempt-${ATTEMPT_3_NO}`);
  ...
  if (manifest.attemptNo !== ATTEMPT_3_NO) {
  ...
  if (
    typeof roots !== 'object' ||
    roots === null ||
    JSON.stringify(Object.keys(roots).sort()) !== JSON.stringify([ATTEMPT_3_VARIANT_NAME])
  ) {
    fail('the experiment manifest names variant roots other than the one V4 root.');
  }
  if (
  ...
  if (authorisationSha256 === SPENT_ATTEMPT_1_AUTHORISATION_SHA256) {
    fail('the attempt-3 experiment was driven by the spent attempt-1 authorisation; refused.');
  }
  if (authorisationSha256 === SPENT_ATTEMPT_2_AUTHORISATION_SHA256) {
    fail('the attempt-3 experiment was driven by the spent attempt-2 authorisation; refused.');
  }
  ...
  if (JSON.stringify(completionVariants) !== JSON.stringify([ATTEMPT_3_VARIANT_NAME])) {
    fail(
      `the completion counts variants ${completionVariants.join(', ')}; only ${ATTEMPT_3_VARIANT_NAME} may appear.`,
    );
  }
  if (
    completion.perVariantEndedWithoutStop[ATTEMPT_3_VARIANT_NAME] !== plan.plannedLogicalEvaluations
  ) {
  ...
  if (marker.attemptNo !== ATTEMPT_3_NO) {
  ...
  if (repairInventory.count !== repairArtifactsVerified) {
    fail(
      `the attempt-3 root holds ${repairInventory.count} repair files; ${repairArtifactsVerified} were verified.`,
    );
  }
  artifactsVerified += 1; // the consumption marker.
  if (inventory.count !== ATTEMPT3_EXPECTED_PRIMARY_ARTIFACT_COUNT) {
    fail(
      `the attempt-3 root holds ${inventory.count} primary files; ${ATTEMPT3_EXPECTED_PRIMARY_ARTIFACT_COUNT} are expected.`,
    );
  }
  if (artifactsVerified !== ATTEMPT3_EXPECTED_PRIMARY_ARTIFACT_COUNT) {
    fail(
      `verified ${artifactsVerified} primary artifacts; ${ATTEMPT3_EXPECTED_PRIMARY_ARTIFACT_COUNT} are expected.`,
    );
  }
