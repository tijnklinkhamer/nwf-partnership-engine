/**
 * PHASE 2B-2D — A3 R25: THE COMMITTED A2 GOVERNANCE V2 AUTHORITY ADAPTER.
 *
 * ONE QUESTION
 *
 *   From the exact committed governance Registry V2 names - read from pinned
 *   commits, never from a working tree - what normalised input does R17
 *   receive, and what slot authority does the REAL R17 derive from it?
 *
 * WHAT THIS MODULE DOES, IN ORDER
 *
 *   1. verifies every Registry V2 entry from its own commit (`commitLoader.ts`);
 *   2. normalises the frozen draw, digesting each entry with the already-landed
 *      canonical primitive;
 *   3. validates the nine-entry replacement ledger with the CANONICAL
 ...
 *      V1 validator, over the verified LEGACY SUBVIEW;
 *   5. parses every V1-compatible record with the UNCHANGED V1 family parsers
 *      over that same subview, and the one new record with its own parser
 *      (`familiesV2.ts`);
 *   6. scopes every supersession chain to one occupancy episode;
 *   7. audits all nine replacement-ledger entries against committed authority
 *      that predates each entry - and emits NONE of those historical facts;
 *   8. calls the real R17 resolver and the real R17 summary.
 *
 * WHY STEPS 2, 3, 6 AND 7 ARE RESTATED HERE
 *
 *   They are R19 ADAPTER logic (not R17 logic), and R19's versions are private
 *   to the V1 namespace, which stays byte-identical so that Registry V1 keeps
 *   meaning exactly the R19 snapshot. Each is the same algorithm over the same
 ...
 *   "latest", and no SET_P / SET_R / SD7 / SD9 / corpus step.
 */
import {
...
function normaliseDraw(governance: CommittedGovernanceV2): NormalisedDraw {
  const file = requireCommittedFile(governance, 'FROZEN_DRAW_V2_GEN1');
...
// ---------------------------------------------------------------------------
// B. THE NINE-ENTRY REPLACEMENT LEDGER.
// ---------------------------------------------------------------------------
...
function normaliseReplacementLedger(
  ...
  const file = requireCommittedFile(governance, REPLACEMENT_LEDGER_REGISTRY_ID);
  ...
  if (
    raw.ledgerHash !== REPLACEMENT_LEDGER_V2_PIN.ledgerHash ||
    entryCount !== REPLACEMENT_LEDGER_V2_PIN.entryCount
  ) {
    refuseV2(
      'REPLACEMENT_LEDGER_V2_PIN_MISMATCH',
      'the validated ledger is not the revision Registry V2 pins',
    );
  }
...
interface ScopedSupersessionV2 {
...
export interface ReplacementHistoryAuditEntryV2 {
...
export interface ReplacementHistoryAuditV2 {
  readonly auditedEntryCount: number;
  readonly entries: readonly ReplacementHistoryAuditEntryV2[];
}
...
): ReplacementHistoryAuditV2 {
  const entries = ledger.revision.entries;
  const audited = entries.map((entry): ReplacementHistoryAuditEntryV2 => {
...
export interface CommittedGovernanceResolutionV2 {
  ...
  readonly scopedSupersessions: readonly ScopedSupersessionV2[];
  readonly replacementHistoryAudit: ReplacementHistoryAuditV2;
  readonly v2FamilyTerminalItems: readonly ParsedTerminalItem[];
...
/**
 * Resolves Registry V2 from committed objects. The registry is a parameter
 ...
 * point is `snapshotV2.ts`, which accepts no registry at all.
 */
export function resolveCommittedA2GovernanceV2(
  repositoryRoot: string,
  registry: readonly GovernanceRegistryEntryV2[] = COMMITTED_A2_GOVERNANCE_REGISTRY_V2_ENTRIES,
): CommittedGovernanceResolutionV2 {
  const governance = loadCommittedGovernanceV2(repositoryRoot, registry);
  const legacy = legacyV1Subview(governance);
  ...
  const v2Family = parsePostP18SecondWindowAdjudication(governance);
  const parsed = mergeFamilyOutputs([parseGovernanceFamilies(legacy), v2Family]);
  ...
  const supersessionByEpisode = new Map<string, ScopedSupersessionV2>();
  ...
  const tipBinding = bindingOfCommitted(requireCommittedFile(governance, TRANSITION_LEDGER_TIP_V1));
  ...
  for (const episode of current) {
    ...
    if (found.length === 0) {
      if (frozenReasonsByEpisode.has(key) || runClaimsByEpisode.has(key)) {
        refuse(
          ...
          `selection slot ${String(episode.selectionIndex)}'s current occupant is named by registered evidence with no terminal adjudication in Registry V2`,
        );
      }
      continue;
    }
    ...
    adjudications.push(
      Object.freeze({
        ...
        adjudication: bindingOfCommitted(requireCommittedFile(governance, item.sourceRegistryId)),
        liveResult: bindingOfCommitted(requireCommittedFile(governance, item.liveResultRegistryId)),
  ...
  const resolution = resolveGenerationSlotAuthorities({
    ...
    // terminally adjudicated, so nothing is pending. Historical live results
    // are not emitted merely because they exist.
  ...
  return Object.freeze({
    ...
    v2FamilyTerminalItems: v2Family.terminalItems,
...
export type { ScopedSupersessionV2 };
