/**
 * PHASE 2B-2D A3 R30 — ISOLATION OF THE INCREMENTAL REACHABLE-MEMBERSHIP / SD9 READINESS.
 *
 * By reading source text, the repository's own history and the committed
 * records, this file proves:
 *
 *   - R30 lives in the NEW sibling namespace `a3readinessV2/`, and every
 *     earlier A3 namespace, the canonical SD7 modules and every R19-R29 public
 *     census and audit are byte-identical to the canonical R29 tip, each
 *     pinned by sha256 - R24's `a3readiness/` and the canonical `a3prep/`
 *     preflight and contracts above all;
 *   - R30 reaches readiness semantics ONLY through R24's
 *     `deriveUnboundSlotReachableMembershipReadiness`, its input ONLY through
 *     R29's brands and provenance accessors, and never names a direct SD9
 *     call, a readiness derivation, a cap determination, a complete-corpus
 *     preflight, R24 V1 minting or any database entry point;
 *   - no environment, filesystem, network, provider, classifier, sealed-root,
 *     child-process or SQL capability exists in the namespace, no cap is
 *     sliced, and no R30 type copies a document identity;
 *   - the only pre-existing file R30 touched is R29's historical scope pin;
 *   - the committed R30 census discloses no identity and invents no digest.
 */
...
import { R30_PUBLIC_CENSUS_RECORD_KIND } from '../harness/phase2b2d/a3readinessV2/census.js';

const REPO_ROOT = resolve(__dirname, '..', '..', '..');
const HARNESS = 'src/test/harness/phase2b2d';
const A3READINESS_V2_REL = `${HARNESS}/a3readinessV2`;

/** The exact canonical R29 tip R30 was cut from. */
const R29_TERMINAL = '566a1bd2d4848e5319326a86453e5db9cdc5d77e';

/**
 * R30'S OWN TERMINAL COMMIT.
 *
 * R30's changed-surface and docs-scope assertions describe R30'S SLICE, so
 * they range over R30's own commits - `R29_TERMINAL..R30_TERMINAL` - rather
 * than over the working tree. Once a later slice lands on top, the working
 * tree is no longer R30's surface, and diffing to it would fail for the
 * honest reason that history moved on rather than because R30 changed.
 *
 * This is the same standing convention R19 through R29 apply, and it
 * WEAKENS NOTHING: R30's range is frozen, its permitted-path list is
...
/** The one commit that pinned R29's own changed-surface test to its range. */
const R29_SCOPE_PIN_COMMIT = '4ec3bf8bb2538fa695b0ec3e0f9a2e51f5d9493b';
const R29_ISOLATION_TEST = 'src/test/unit/orgunitCorpus2DA3SampleSurvivorV2Isolation.test.ts';

const R30_TESTS = [
  'src/test/unit/orgunitCorpus2DA3ReachableMembershipSd9V2Delta.test.ts',
...
/** Namespaces R30 must leave byte-identical: every file is pinned below. */
 ...
 * R19-R29 public census and audit, as their bytes stand at the R29 tip.
 */
const FROZEN_SHA256_AT_R29: Readonly<Record<string, string>> = {
...
function code(file: string): string {
  return stripComments(readFileSync(join(REPO_ROOT, A3READINESS_V2_REL, file), 'utf8'));
}
...
function allCode(): string {
  return A3READINESS_V2_FILES.map(code).join('\n');
}
...
const baseAvailable =
  commitExists(R29_TERMINAL) && commitExists(R29_SCOPE_PIN_COMMIT) && commitExists(R30_TERMINAL);

// ---------------------------------------------------------------------------

describe('2D-A3 R30: the new namespace is exactly these files', () => {
  it('holds the six incremental readiness modules', () => {
    expect(readdirSync(join(REPO_ROOT, A3READINESS_V2_REL)).sort()).toEqual(A3READINESS_V2_FILES);
  });
  ...
  it('pins every file of every frozen namespace, and no file was added to one', () => {
    for (const namespace of FROZEN_NAMESPACES) {
      const onDisk = readdirSync(join(REPO_ROOT, HARNESS, namespace))
        ...
        .sort();
      const pinned = Object.keys(FROZEN_SHA256_AT_R29)
  ...
  it('leaves every pinned implementation file, census and audit byte-identical', () => {
    for (const [path, digest] of Object.entries(FROZEN_SHA256_AT_R29)) {
      expect(sha256(readFileSync(join(REPO_ROOT, path))), path).toBe(digest);
    }
  });

  it("§7: names R24's membership/types/devTrain/census and the canonical preflight and contracts", () => {
    for (const path of [
      `${HARNESS}/a3readiness/membership.ts`,
      `${HARNESS}/a3readiness/types.ts`,
      `${HARNESS}/a3readiness/devTrain.ts`,
      `${HARNESS}/a3readiness/census.ts`,
      `${HARNESS}/a3prep/corpusFreezePreflight.ts`,
      `${HARNESS}/a3prep/contracts.ts`,
      `${HARNESS}/a3samplesV2/devTrain.ts`,
      'docs/evaluation/PHASE_2B_2D_A3_R24_DEV_TRAIN_REACHABLE_MEMBERSHIP_SD9_READINESS_CENSUS_V1.json',
      'docs/evaluation/PHASE_2B_2D_A3_R29_DEV_TRAIN_INCREMENTAL_SAMPLE_SURVIVOR_PREPARATION_CENSUS_V1.json',
      'docs/audits/PHASE_2B_2D_A3_R29_INCREMENTAL_DEV_TRAIN_SAMPLE_SURVIVOR_PREPARATION_V1.md',
    ]) {
      expect(FROZEN_SHA256_AT_R29[path], path).toMatch(/^[0-9a-f]{64}$/);
    }
  });
});

describe("2D-A3 R30 §1-§3 / §50: R24's pure helper and R29's provenance, nothing else", () => {
  const PERMITTED_SPECIFIERS = new Set([
    '../a3readiness/membership.js',
    '../a3readiness/types.js',
    '../a3samplesV2/devTrain.js',
    '../a3samplesV2/types.js',
    '../a3samplesV2/census.js',
  ...
  it('imports nothing but R24 membership/types, R29, the V2 snapshot type and local modules', () => {
    for (const file of A3READINESS_V2_FILES) {
  ...
  it('§50: imports nothing at all from a3prep/, sd7/, R23 or any earlier upstream layer', () => {
    expect(allCode()).not.toMatch(
      /a3prep\/|sd7\/|a3samples\/|a3graphs|a3documents|a3evidence|a3governance\//,
    );
  });
  ...
  it('§16: calls the R24 helper in exactly one place, with the exact R29 preparation', () => {
    const source = allCode();
    ...
    expect(source.match(/deriveDeltaSlotReadinessAllOrNothing\(/g)).toHaveLength(2);
  });

  it('takes from R29 only its brands, provenance accessors and split constant', () => {
    const source = allCode();
    expect(importsFrom(source, '../a3samplesV2/devTrain.js').runtime).toEqual([
    ...
    expect(importsFrom(source, '../a3samplesV2/types.js').runtime).toEqual(['R29_SAMPLE_SPLIT']);
    expect(importsFrom(source, '../a3samplesV2/census.js').runtime).toEqual([]);
    expect(importsFrom(source, '../a3governanceV2/snapshotV2.js').runtime).toEqual([]);
  });

  it('§2 / §15 / §50 / §56: names no direct SD9, readiness, cap, preflight, historical or upstream call', () => {
    ...
    for (const forbidden of [
      ...
      'isA3DevTrainReachableMembershipSd9Batch\\b',
      'isA3DevTrainSlotReachableMembershipReadiness\\b',
...
describe('2D-A3 R30: no env, fs, network, SQL, provider, sealed, slicing or extension capability', () => {
  ...
  it('§9: writes no SQL and opens no transaction', () => {
    const source = allCode();
  ...
  it('§3 / §56-§58: slices no cap, sorts no document, and owns no SD4/K4 or extension code', () => {
    // r29Drift.ts sorts JSON record KEYS for its drift comparison; it touches no document.
    const source = A3READINESS_V2_FILES.filter((file) => file !== 'r29Drift.ts')
      .map(code)
      .join('\n')
      .replace(/'[^'\n]*'/g, "''");
    expect(allCode()).not.toMatch(/\.slice\s*\(|\.splice\s*\(|\.subarray\s*\(/);
    expect(code('r29Drift.ts')).not.toMatch(/documentSha256|measurableSurvivors|documents/);
    for (const forbidden of [
      /greedy\w*\s*\(|survivorWalk\s*\(|walkSurvivors|nearDuplicatePass/i,
      /salt\w*\s*\(|compareScore|sortBy|\.sort\s*\(/,
      /Sd9Bounds|evaluateSd9|minPagesNotMet\s*\(/i,
  ...
  it('§32: no R30 type or minted object copies a document identity, text, score or edge', () => {
    ...
    expect(allCode()).not.toMatch(/mainText|candidateScoreDecimal/);
  });

  it('§66: invents no readiness, membership, SD9 or corpus digest', () => {
    const source = allCode();
  ...
  it('§15 / §41: exposes no entry point over several organisations or historical readiness', () => {
    expect(allCode()).not.toMatch(
      /historicalReadiness\w*\s*\(|historicalPreparations|allSlots|sixSlot\w*\s*\(|combinedDocuments|globalDocuments/,
    );
  });
});

describe.skipIf(!baseAvailable)('2D-A3 R30: lineage and changed surface', () => {
  it('descends from the exact canonical R29 tip, and merges no A2 commit', () => {
    expect(() => git('merge-base', '--is-ancestor', R29_TERMINAL, 'HEAD')).not.toThrow();
    expect(() => git('merge-base', '--is-ancestor', R29_SCOPE_PIN_COMMIT, 'HEAD')).not.toThrow();
    expect(lines(git('rev-list', '--merges', `${R29_TERMINAL}..HEAD`))).toEqual([]);
    for (const a2 of [
  ...
  it('pinned R29 scope in exactly one commit that touched exactly one file', () => {
    expect(lines(git('diff', '--name-only', R29_TERMINAL, R29_SCOPE_PIN_COMMIT))).toEqual([
      R29_ISOLATION_TEST,
    ]);
    expect(sha256(readFileSync(join(REPO_ROOT, R29_ISOLATION_TEST)))).toBe(
      sha256(git('show', `${R29_SCOPE_PIN_COMMIT}:${R29_ISOLATION_TEST}`)),
    );
  });

  it('leaves every frozen namespace untouched since the R29 tip', () => {
    for (const namespace of FROZEN_NAMESPACES) {
      const path = `${HARNESS}/${namespace}`;
      expect(lines(git('diff', '--name-only', R29_TERMINAL, '--', path)), path).toEqual([]);
  ...
  it('writes no governance record: every docs/evaluation change is the R30 census', () => {
    const changed = lines(
      git('diff', '--name-only', R29_TERMINAL, R30_TERMINAL, '--', 'docs/evaluation'),
    );
    expect(changed.filter((path) => path !== R30_CENSUS_PATH)).toEqual([]);
  });

  it('changes nothing outside its own namespace, tests, records and the R29 scope pin', () => {
    const paths = lines(git('diff', '--name-only', R29_TERMINAL, R30_TERMINAL));
    const permitted = (path: string): boolean =>
      path.startsWith(`${A3READINESS_V2_REL}/`) ||
      R30_TESTS.includes(path) ||
      path === R29_ISOLATION_TEST ||
      path === R30_CENSUS_PATH ||
      path === R30_AUDIT_PATH;
    expect(paths.filter((path) => !permitted(path))).toEqual([]);
  });
});

describe.skipIf(!existsSync(join(REPO_ROOT, R30_CENSUS_PATH)))(
  '2D-A3 R30: the public census discloses nothing',
  () => {
    const raw = existsSync(join(REPO_ROOT, R30_CENSUS_PATH))
      ? readFileSync(join(REPO_ROOT, R30_CENSUS_PATH), 'utf8')
      : '{}';
    ...
    const sample = (docs: number) => ({
      reachableMembershipExactSlotCount: 1,
      ...
      measurableSurvivorCount: 34,
      unresolvedShortTextOccurrenceCount: 0,
      sd9MinEnvelopeTotal: 34,
      sd9MaxEnvelopeTotal: 34,
      sd9MechanicalSuccessfulSlotCount: 1,
      sd9MechanicalUnsuccessfulSlotCount: 0,
      sd9MechanicalPendingSlotCount: 0,
    });
    const combined = (docs: number) => ({
    ...
    it('is the derived census record, and authorises nothing', () => {
      expect(parsed['record']).toBe(R30_PUBLIC_CENSUS_RECORD_KIND);
      ...
      expect(parsed['r29Tip']).toBe(R29_TERMINAL);
    });

    it('§61: one delta slot, exact 8 / 4, [34,34] successful in both samples', () => {
      const delta = parsed['delta'] as Record<string, unknown>;
      expect(delta['deltaReadinessSlots']).toBe(1);
      expect(delta['setP']).toEqual(sample(8));
      expect(delta['setR']).toEqual(sample(4));
      expect(delta['crossSample']).toEqual({
        bothSamplesMechanicallySuccessfulSlotCount: 1,
        sd9StatusDisagreementSlotCount: 0,
      });
    });

    it('§62: 5 historical + 1 new = 6 slots, and combined readiness coverage', () => {
      const coverage = parsed['coverage'] as Record<string, unknown>;
      expect(coverage['historicalCanonicalR24ReadinessSlots']).toBe(5);
      expect(coverage['newR30ReadinessSlots']).toBe(1);
      expect(coverage['coverageReadinessSlots']).toBe(6);
      expect(coverage['sixSlotReadinessBatchMinted']).toBe(false);
      const all = coverage['coverage'] as Record<string, unknown>;
      expect(all['setP']).toEqual(combined(48));
      expect(all['setR']).toEqual(combined(24));
      expect(all['crossSample']).toEqual({
        bothSamplesMechanicallySuccessfulSlotCount: 6,
        sd9StatusDisagreementSlotCount: 0,
      });
    });

    it('§63: states the owner-policy tokens, caps 8 / 4, SD9 min pages 4 and the semantics token', () => {
      const c = parsed['canonicalConstants'] as Record<string, unknown>;
      expect(c['reachableMembershipDecisionToken']).toBe(
        'SHORT_TEXT_REQUIRED_SAMPLE_MEMBERSHIP_LIMITED_TO_REACHABLE_CAPPED_MEMBERSHIP_V1',
      );
      expect(c['generation']).toBe('METHODOLOGY_V2_GEN1');
      expect(c['requiredMembershipScope']).toBe('REACHABLE_SELECTED_CAPPED_MEMBERSHIP');
      expect(c['zeroExtensionHeadroomScope']).toBe(
        'ZERO_EXTENSION_HEADROOM_SELECTED_CAP_IS_COMPLETE_REACHABLE_SAMPLE_MEMBERSHIP',
      );
      expect(c['unreachableTailFreezeEffect']).toBe('DOES_NOT_REFUSE_FREEZE_BY_ITSELF');
      expect(c['setPMaxPagesPerOrganisation']).toBe(8);
      expect(c['setRMaxPagesPerOrganisation']).toBe(4);
      expect(c['sd9MinPagesPerOrganisation']).toBe(4);
      expect(c['sd9Semantics']).toBe(
        'A3_MECHANICAL_SD9_OF_CANONICAL_SURVIVOR_ENVELOPE_ONLY_NOT_A2_ACQUISITION_STATUS',
      );
    });

    it('§55 / §64: states delta-only, canonical-R24-only, mechanical-only semantics and zero old-five work', () => {
      ...
      const boundary = parsed['sd9AuthorityBoundary'] as Record<string, unknown>;
      ...
      expect(access['r30SqlStatements']).toBe(0);
      ...
      expect(access['oldFiveReadinessDerivation']).toBe(0);
    });

    it('§65: carries no identity-, position-, digest-, membership-, text- or per-slot-bearing key', () => {
      for (const key of [
        'selectionIndex',
        'selectionIndices',
        'organisationId',
    ...
    it('names no URL, domain, sealed filename or run-shaped digest', () => {
      ...
      const permitted = new Set([R29_TERMINAL, String(parsed['implementationCommit'])]);
