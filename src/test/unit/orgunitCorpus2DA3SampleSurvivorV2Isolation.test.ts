...
/**
 * R29'S OWN TERMINAL COMMIT.
 *
 * R29's changed-surface and docs-scope assertions describe R29'S SLICE, so
 * they range over R29's own commits - `R28_TERMINAL..R29_TERMINAL` - rather
 * than over the working tree. Once a later slice lands on top, the working
 * tree is no longer R29's surface, and diffing to it would fail for the
 * honest reason that history moved on rather than because R29 changed.
 *
 * This is the same standing convention R19 through R28 apply, and it
 * WEAKENS NOTHING: R29's range is frozen, its permitted-path list is
...
const baseAvailable =
  commitExists(R28_TERMINAL) && commitExists(R28_SCOPE_PIN_COMMIT) && commitExists(R29_TERMINAL);
...
describe.skipIf(!baseAvailable)('2D-A3 R29: lineage and changed surface', () => {
  ...
  it('writes no governance record: every docs/evaluation change is the R29 census', () => {
    const changed = lines(
      git('diff', '--name-only', R28_TERMINAL, R29_TERMINAL, '--', 'docs/evaluation'),
    );
  ...
  it('changes nothing outside its own namespace, tests, records and the R28 scope pin', () => {
    const paths = lines(git('diff', '--name-only', R28_TERMINAL, R29_TERMINAL));
